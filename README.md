# python base 
## when you have a little python or code knowledge,then you can do it.

http://github.com/ehmatthes/pcc_3e

 chapter01-chapter03 python print str | f'str' | simple list
 
 chapter 04 python list | tuple | slice | comprehensions
 
 chapter 05-chapter06 if-elif-else | for | dict with set
 
 chapter 07 while loop | list | if-else 
 
 chapter 08 define function with *variable **variable | list 
 
 chapter 09 use class by create object's attributes and methods
